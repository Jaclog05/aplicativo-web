export const questionsMultiFamiliesArray = [
  {
    "id": 1,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Accesibilidad",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer el estado (bueno, regular o malo) de las vías vehiculares y peatonales de acceso hasta la entrada principal de la unidad residencial. También la presencia de baldosas podotáctiles y rampas que ayudan al acceso fácil e independiente de las personas con discapacidad.",
    "explanation": "Elementos o espacios que nos permiten acceder a la vivienda, (Vías vehiculares, andenes, escaleras, ascensores, rampas, entre otros).",
    "question": "¿En qué estado se encuentran las vías vehiculares de acceso?",
    "options": {
      "Malo": 0,
      "Bueno": 0.015,
      "Regular": 0.0075
    },
    "imageId": 1,
    "parameterType": "Regular"
  },
  {
    "id": 2,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Accesibilidad",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer el estado (bueno, regular o malo) de las vías vehiculares y peatonales de acceso hasta la entrada principal de la unidad residencial. También la presencia de baldosas podotáctiles y rampas que ayudan al acceso fácil e independiente de las personas con discapacidad.",
    "explanation": "Elementos o espacios que nos permiten acceder a la vivienda, (Vías vehiculares, andenes, escaleras, ascensores, rampas, entre otros).",
    "question": "¿En qué estado se encuentran las vías peatonales de acceso?",
    "options": {
      "Malo": 0,
      "Bueno": 0.015,
      "Regular": 0.0075
    },
    "imageId": 2,
    "parameterType": "Regular"
  },
  {
    "id": 3,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Accesibilidad",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer el estado (bueno, regular o malo) de las vías vehiculares y peatonales de acceso hasta la entrada principal de la unidad residencial. También la presencia de baldosas podotáctiles y rampas que ayudan al acceso fácil e independiente de las personas con discapacidad.",
    "explanation": "Elementos o espacios que nos permiten acceder a la vivienda, (Vías vehiculares, andenes, escaleras, ascensores, rampas, entre otros).",
    "question": "¿En el espacio público los andenes cuentan con rampas para discapacitados?",
    "options": {
      "No": 0,
      "Si": 0.015
    },
    "imageId": 3,
    "parameterType": "Regular"
  },
  {
    "id": 4,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Accesibilidad",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer el estado (bueno, regular o malo) de las vías vehiculares y peatonales de acceso hasta la entrada principal de la unidad residencial. También la presencia de baldosas podotáctiles y rampas que ayudan al acceso fácil e independiente de las personas con discapacidad.",
    "explanation": "Elementos o espacios que nos permiten acceder a la vivienda, (Vías vehiculares, andenes, escaleras, ascensores, rampas, entre otros).",
    "question": "¿En el espacio público los andenes cuentan con baldosas podotáctiles?",
    "options": {
      "No": 0,
      "Si": 0.015
    },
    "imageId": 4,
    "parameterType": "Regular"
  },
  {
    "id": 5,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Zona de riesgo",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer si fenómenos naturales (arroyos), industria (emoción de gases) o alto flujo vehicular (contaminación auditiva) afectan directamente a la unidad residencial. En este caso, la presencia de estos factores resulta en la disminución de la calidad. A su vez se toman en cuenta, factores de seguridad como la presencia de rutas de evacuación adecuadas en caso de incendio.",
    "explanation": "Elementos que representen una amenaza en el sitio ya sea que afecten la movilidad, la salud del residente o su seguridad.",
    "question": "A un máximo de 5 manzanas a la redonda de la vivienda ¿se presentan corrientes de agua (arroyos)?",
    "options": {
      "No": 0.02,
      "Si": 0
    },
    "imageId": 5,
    "parameterType": "Regular"
  },
  {
    "id": 6,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Zona de riesgo",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer si fenómenos naturales (arroyos), industria (emoción de gases) o alto flujo vehicular (contaminación auditiva) afectan directamente a la unidad residencial. En este caso, la presencia de estos factores resulta en la disminución de la calidad. A su vez se toman en cuenta, factores de seguridad como la presencia de rutas de evacuación adecuadas en caso de incendio.",
    "explanation": "Elementos que representen una amenaza en el sitio ya sea que afecten la movilidad, la salud del residente o su seguridad.",
    "question": "A un máximo de 5 manzanas a la redonda de la vivienda ¿se encuentran industrias o fábricas que emiten gases nocivos para el medio ambiente?",
    "options": {
      "No": 0.02,
      "Si": 0
    },
    "imageId": 6,
    "parameterType": "Regular"
  },
  {
    "id": 7,
    "indicator": "Entorno",
    "subgroup": "Accesibilidad",
    "parameter": "Zona de riesgo",
    "measurement": "Las preguntas orientadoras van enfocadas a conocer si fenómenos naturales (arroyos), industria (emoción de gases) o alto flujo vehicular (contaminación auditiva) afectan directamente a la unidad residencial. En este caso, la presencia de estos factores resulta en la disminución de la calidad. A su vez se toman en cuenta, factores de seguridad como la presencia de rutas de evacuación adecuadas en caso de incendio.",
    "explanation": "Elementos que representen una amenaza en el sitio ya sea que afecten la movilidad, la salud del residente o su seguridad.",
    "question": "¿Cuál es el nivel de tráfico vehicular en la vía principal más cercana a la vivienda?",
    "options": {
      "Alto": 0,
      "Bajo": 0.02,
      "Medio": 0.01
    },
    "imageId": 7,
    "parameterType": "Regular"
  },
  {
    "id": 8,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a escala barrial",
    "measurement": "Este parámetro se enfoca en conocer si a un máximo de 3 manzanas de distancia se encuentra una droguería o un lugar de abastecimiento (tienda) donde los usuarios puedan acceder a ellos rápidamente",
    "explanation": "Espacios o edificaciones de pequeña escala que se deben encontrar en las inmediaciones de la vivienda (Ejemplo: Droguerías, tiendas de barrio, entre otros)",
    "question": "A un máximo de 3 manzanas a la redonda de la vivienda ¿se encuentra un lugar de abastecimiento (tienda de barrio)?",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 8,
    "parameterType": "Regular"
  },
  {
    "id": 9,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a escala barrial",
    "measurement": "Este parámetro se enfoca en conocer si a un máximo de 3 manzanas de distancia se encuentra una droguería o un lugar de abastecimiento (tienda) donde los usuarios puedan acceder a ellos rápidamente",
    "explanation": "Espacios o edificaciones de pequeña escala que se deben encontrar en las inmediaciones de la vivienda (Ejemplo: Droguerías, tiendas de barrio, entre otros)",
    "question": "A un máximo de 3 manzanas a la redonda de la vivienda ¿se encuentra una farmacia/droguería?",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 9,
    "parameterType": "Regular"
  },
  {
    "id": 10,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 10 manzanas a la redonda de la vivienda ¿se encuentran centros educativos?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 10,
    "parameterType": "Regular"
  },
  {
    "id": 11,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "El parámetro va enfocado a conocer si en la localidad existen espacios como plazas y parques, hospitales, supermercados, escuelas o universidades, estaciones de policía o iglesias. A su vez, conocer si la vivienda cuenta con diversas rutas de buses de fácil y rápido acceso (a máximo 3 manzanas)",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 10 manzanas a la redonda de la vivienda ¿se encuentra un supermercado?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 11,
    "parameterType": "Regular"
  },
  {
    "id": 12,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "El parámetro va enfocado a conocer si en la localidad existen espacios como plazas y parques, hospitales, supermercados, escuelas o universidades, estaciones de policía o iglesias. A su vez, conocer si la vivienda cuenta con diversas rutas de buses de fácil y rápido acceso (a máximo 3 manzanas)",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 3 manzanas a la redonda de la vivienda ¿transita por lo menos una ruta de bus?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 12,
    "parameterType": "Regular"
  },
  {
    "id": 13,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "El parámetro va enfocado a conocer si en la localidad existen espacios como plazas y parques, hospitales, supermercados, escuelas o universidades, estaciones de policía o iglesias. A su vez, conocer si la vivienda cuenta con diversas rutas de buses de fácil y rápido acceso (a máximo 3 manzanas)",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 4 manzanas a la redonda de la vivienda ¿se encuentan parques o plazas?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 13,
    "parameterType": "Regular"
  },
  {
    "id": 14,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "El parámetro va enfocado a conocer si en la localidad existen espacios como plazas y parques, hospitales, supermercados, escuelas o universidades, estaciones de policía o iglesias. A su vez, conocer si la vivienda cuenta con diversas rutas de buses de fácil y rápido acceso (a máximo 3 manzanas)",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 10 manzanas a la redonda de la vivienda ¿encuentras un establecimiento donde te brinden asistencia medica?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 14,
    "parameterType": "Regular"
  },
  {
    "id": 15,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "El parámetro va enfocado a conocer si en la localidad existen espacios como plazas y parques, hospitales, supermercados, escuelas o universidades, estaciones de policía o iglesias. A su vez, conocer si la vivienda cuenta con diversas rutas de buses de fácil y rápido acceso (a máximo 3 manzanas)",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 10 manzanas a la redonda de la vivienda ¿encuentras una estación o CAI (Comando de Atención Inmediata) de la policia nacional?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 15,
    "parameterType": "Regular"
  },
  {
    "id": 16,
    "indicator": "Entorno",
    "subgroup": "Equipamientos",
    "parameter": "Equipamientos a nivel ciudad",
    "measurement": "El parámetro va enfocado a conocer si en la localidad existen espacios como plazas y parques, hospitales, supermercados, escuelas o universidades, estaciones de policía o iglesias. A su vez, conocer si la vivienda cuenta con diversas rutas de buses de fácil y rápido acceso (a máximo 3 manzanas)",
    "explanation": "Edificaciones o espacios que cuentan con una mayor cobertura, es decir, que pueden estar mucho más lejanos a la vivienda, pero aún el servicio que ofrecen sigue siendo efectivo (Ejemplo: Centros comerciales, hospitales, plazas, supermercados, parques, entre otros).",
    "question": "A un máximo de 10 manzanas a la redonda de la vivienda ¿encuentras un lugar de culto?",
    "options": {
      "No": 0,
      "Si": 0.003
    },
    "imageId": 16,
    "parameterType": "Regular"
  },
  {
    "id": 17,
    "indicator": "Entorno",
    "subgroup": "Preferencias del usuario",
    "parameter": "Trabajo",
    "measurement": "Este parámetro buscar dar a conocer al usuario la cercanía entre la unidad residencial y su lugar de trabajo y tomarlo como uno de los factores a tener en cuenta a la hora de evaluar las calidad de la vivienda",
    "explanation": "El parámetro se basa en evaluar si la vivienda se encuentra cerca o lejos del lugar de trabajo del usuario. ",
    "question": "¿Tu lugar de trabajo se encuentra a un máximo de 15 minutos de distancia de la vivienda? (Independientemente de tu medio de transporte)",
    "options": {
      "No": 0,
      "Si": 1
    },
    "imageId": 17,
    "parameterType": "Plus",
    "parameterPlusTitle": "Cercania al lugar de trabajo"
  },
  {
    "id": 18,
    "indicator": "Entorno",
    "subgroup": "Percepción del usuario",
    "parameter": "Entorno inmediato",
    "measurement": "Este parámetro busca tener en cuenta las sensaciones del usuario en el barrio en el que se encuentra la vivienda",
    "explanation": "Contexto de la vivienda analizado desde una escala barrial. Es todo lo que rodea la obra arquitectónica (unidad residencial) y la afecta directamente en una distancia aproximada de 500 metros a la redonda. ",
    "question": "¿Qué percibes del barrio donde se encuentra la vivienda?",
    "options": {
      "Es agradable": 1,
      "No me agrada": 0,
      "No es agradable ni desagradable": 0.5
    },
    "imageId": 18,
    "parameterType": "Plus",
    "parameterPlusTitle": "Percepción subjetiva del entorno"
  },
  {
    "id": 19,
    "indicator": "Entorno",
    "subgroup": "Percepción del usuario",
    "parameter": "Vigilancia del sector",
    "measurement": "Para evaluar la vigilancia del sector, observa si hay presencia de casetas de seguridad, guardias, cámaras visibles o patrullaje policial durante tus recorridos habituales o en una sola visita. También puedes preguntar a vecinos cercanos si hay vigilancia contratada o comunitaria activa en la zona.",
    "explanation": "Medidas y sistemas implementados para garantizar la seguridad del entorno de la vivienda, tales como vigilancia privada, cámaras de seguridad o patrullaje policial.",
    "question": "¿El sector cuenta con algún tipo de vigilancia?",
    "options": {
      "No": 0,
      "Si": 1,
      "A veces": 0.5
    },
    "imageId": 19,
    "parameterType": "Plus",
    "parameterPlusTitle": "Seguridad del sector"
  },
  {
    "id": 20,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Espacios Privados",
    "measurement": "Verifica si tu vivienda cuenta con un cuarto útil o bodega asignada, ya sea en el mismo piso del apartamento, en la zona de parqueo u otra área común del conjunto. En caso de duda, consulta con el vendedor o administrador del inmueble para confirmar su existencia y ubicación.",
    "explanation": "Áreas dentro de la vivienda destinadas al uso exclusivo de los residentes, como depósitos, bodegas o cuartos útiles.",
    "question": "¿La vivienda cuenta con depósito, bodega o cuarto útil?",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 20,
    "parameterType": "Regular"
  },
  {
    "id": 21,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Espacios Comunales",
    "measurement": "Realiza un recorrido por el conjunto residencial para identificar las amenidades disponibles, como parque infantil, canchas, gimnasio, salón social, piscina, entre otras. Además, consulta si el conjunto cuenta con planta eléctrica",
    "explanation": "Espacios que permiten la interacción y convivencia entre una comunidad. En este caso, espacios que están disponibles y son compartidos por todos los propietarios e inquilinos del conjunto.",
    "question": "¿El conjunto cuenta con planta eléctrica?",
    "options": {
      "No": 0,
      "Si": 0.012
    },
    "imageId": 21,
    "parameterType": "Regular"
  },
  {
    "id": 22,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Espacios Comunales",
    "measurement": "Realiza un recorrido por el conjunto residencial para identificar las amenidades disponibles, como parque infantil, canchas, gimnasio, salón social, piscina, entre otras. Además, consulta si el conjunto cuenta con planta eléctrica",
    "explanation": "Espacios que permiten la interacción y convivencia entre una comunidad. En este caso, espacios que están disponibles y son compartidos por todos los propietarios e inquilinos del conjunto.",
    "question": "¿Cuenta con espacios de esparcimiento como un parque y una cancha?",
    "options": {
      "No": 0,
      "Si": 0.012
    },
    "imageId": 22,
    "parameterType": "Regular"
  },
  {
    "id": 23,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Espacios Comunales",
    "measurement": "Realiza un recorrido por el conjunto residencial para identificar las amenidades disponibles, como parque infantil, canchas, gimnasio, salón social, piscina, entre otras. Además, consulta si el conjunto cuenta con planta eléctrica",
    "explanation": "Espacios que permiten la interacción y convivencia entre una comunidad. En este caso, espacios que están disponibles y son compartidos por todos los propietarios e inquilinos del conjunto.",
    "question": "¿Cuenta con espacios de reunión como un salón social?",
    "options": {
      "No": 0,
      "Si": 0.012
    },
    "imageId": 23,
    "parameterType": "Regular"
  },
  {
    "id": 24,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Espacios Comunales",
    "measurement": "Realiza un recorrido por el conjunto residencial para identificar las amenidades disponibles, como parque infantil, canchas, gimnasio, salón social, piscina, entre otras. Además, consulta si el conjunto cuenta con planta eléctrica",
    "explanation": "Espacios que permiten la interacción y convivencia entre una comunidad. En este caso, espacios que están disponibles y son compartidos por todos los propietarios e inquilinos del conjunto.",
    "question": "¿Cuenta con gimnasio?",
    "options": {
      "No": 0,
      "Si": 0.012
    },
    "imageId": 24,
    "parameterType": "Regular"
  },
  {
    "id": 25,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Espacios Comunales",
    "measurement": "Realiza un recorrido por el conjunto residencial para identificar las amenidades disponibles, como parque infantil, canchas, gimnasio, salón social, piscina, entre otras. Además, consulta si el conjunto cuenta con planta eléctrica",
    "explanation": "Espacios que permiten la interacción y convivencia entre una comunidad. En este caso, espacios que están disponibles y son compartidos por todos los propietarios e inquilinos del conjunto.",
    "question": "¿Cuenta con una piscina?",
    "options": {
      "No": 0,
      "Si": 0.012
    },
    "imageId": 25,
    "parameterType": "Regular"
  },
  {
    "id": 26,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Mascotas",
    "measurement": "Enfocado a conocer si existe un espacio de uso exclusivo para las mascotas de los residentes, donde puedan hacer sus necesidades fisiológicas y recrearse.",
    "explanation": "Animal domesticado que convive y acompaña al ser humano.",
    "question": "¿Cuenta con espacios destinados para el esparcimiento y uso biológico de las mascotas?",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 26,
    "parameterType": "Regular"
  },
  {
    "id": 27,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Punto Fijo",
    "measurement": "Este parámetro está enfocado en la percepción de los usuarios hacia las áreas de circulación del edificio o la torre (en caso de ser un conjunto de edificios) hasta la entrada principal de la vivienda",
    "explanation": "Áreas o espacios de circulación, ya sean verticales (Ejemplo: ascensores, escaleras y/o rampas) u horizontales (Ejemplo: pasillos) que permiten el tránsito entre los espacios comunales de un conjunto residencial y la unidad residencial (vivienda).",
    "question": "¿Qué percepción te generan los elementos que te permiten acceder a la vivienda (pasillos, senderos, escaleras, rampas, ascensor, entre otros)?",
    "options": {
      "Malo": 0,
      "Bueno": 0.02,
      "Regular": 0.0075
    },
    "imageId": 27,
    "parameterType": "Regular"
  },
  {
    "id": 28,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Manejo de Residuos",
    "measurement": "",
    "explanation": "Proceso de recolección, traslado y acopio de residuos producidos por una vivienda.",
    "question": "¿El piso en el cual se encuentra la vivienda posee shut de basuras?",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 28,
    "parameterType": "Regular"
  },
  {
    "id": 29,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Manejo de Residuos",
    "measurement": "Verifica si existen puntos de recolección de basura, como depósitos o shut de basura, ya sea en el mismo piso o en otras áreas del conjunto. Además, identifica si el conjunto cuenta con algún sistema de clasificación de residuos que promueva su reutilización y contribuya a una ciudad más sostenible",
    "explanation": "Proceso de recolección, traslado y acopio de residuos producidos por una vivienda.",
    "question": "¿El conjunto cuenta con algún sistemas de clasificación de residuos producido por las viviendas?",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 29,
    "parameterType": "Regular"
  },
  {
    "id": 30,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Sistemas de evacuacion",
    "measurement": "Identifica las señales de evacuación en el conjunto, suelen ser de color verde con símbolos blancos, e indican rutas de salida, escaleras o puntos de encuentro en caso de emergencia.",
    "explanation": "Infraestructuras y rutas diseñadas para facilitar la salida segura de los residentes en caso de emergencia, como escaleras de evacuación y salidas de emergencia.",
    "question": "¿El conjunto residencial cuenta con escalera de evacuación?",
    "options": {
      "No": 0,
      "Si": 0.01
    },
    "imageId": 30,
    "parameterType": "Regular"
  },
  {
    "id": 31,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Sistemas de evacuacion",
    "measurement": "Identifica las señales de evacuación en el conjunto, suelen ser de color verde con símbolos blancos, e indican rutas de salida, escaleras o puntos de encuentro en caso de emergencia.",
    "explanation": "Infraestructuras y rutas diseñadas para facilitar la salida segura de los residentes en caso de emergencia, como escaleras de evacuación y salidas de emergencia.",
    "question": "¿El conjunto residencial cuenta con la señalización necesaria para guiar una evacuación de emergencia?",
    "options": {
      "No": 0,
      "Si": 0.01
    },
    "imageId": 31,
    "parameterType": "Regular"
  },
  {
    "id": 32,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Mantenimiento (fachada y espacios comunales)",
    "measurement": "Observa el estado físico general del edificio o conjunto residencial: identifica señales como pintura reciente, fisuras reparadas y paredes limpias. Además, fíjate si hay personal de mantenimiento trabajando o avisos que indiquen labores recientes. Recorre también las zonas comunes y verifica que se encuentren limpias, cuidadas y en buen estado de funcionamiento.",
    "explanation": "Proceso de conservación, limpieza y reparación de las fachadas y las áreas comunes del conjunto residencial para mantener su funcionalidad y estética.",
    "question": "¿En qué estado se encuentran las zonas comunes del conjunto?",
    "options": {
      "Malo": 0,
      "Bueno": 0.02,
      "Regular": 0.01
    },
    "imageId": 32,
    "parameterType": "Regular"
  },
  {
    "id": 33,
    "indicator": "Entorno",
    "subgroup": "Criterios Especificos",
    "parameter": "Mantenimiento (fachada y espacios comunales)",
    "measurement": "Observa el estado físico general del edificio o conjunto residencial: identifica señales como pintura reciente, fisuras reparadas y paredes limpias. Además, fíjate si hay personal de mantenimiento trabajando o avisos que indiquen labores recientes. Recorre también las zonas comunes y verifica que se encuentren limpias, cuidadas y en buen estado de funcionamiento.",
    "explanation": "Proceso de conservación, limpieza y reparación de las fachadas y las áreas comunes del conjunto residencial para mantener su funcionalidad y estética.",
    "question": "¿En qué estado se encuentran las fachadas de la edficación?",
    "options": {
      "Malo": 0,
      "Bueno": 0.02,
      "Regular": 0.01
    },
    "imageId": 33,
    "parameterType": "Regular"
  },
  {
    "id": 34,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Iluminacion natural",
    "measurement": "Las preguntas orientadoras de esté parámetro van enfocadas a que el usuario lleve su atención hacia la cantidad de luz natural qué hay en los diferentes espacios de la vivienda. Se divide entre las zonas comunes (sala, comedor, cocina, etc.) y la zona privada (habitaciones) ya que cada una requiere una cantidad de iluminación diferente",
    "explanation": "Es aquel recurso natural que incide positivamente en los espacios. Se utiliza con el fin de optimizar la utilización de energías no renovables como la iluminación artificial.",
    "question": "¿Cómo percibes la iluminación natural en las zonas comunes dentro de la vivienda? (Sala, comedor, estudio, cocina, entre otros)",
    "options": {
      "Buena": 0.02,
      "Regular": 0.01,
      "Deficiente": 0
    },
    "imageId": 34,
    "parameterType": "Regular"
  },
  {
    "id": 35,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Iluminacion natural",
    "measurement": "Las preguntas orientadoras de esté parámetro van enfocadas a que el usuario lleve su atención hacia la cantidad de luz natural qué hay en los diferentes espacios de la vivienda. Se divide entre las zonas comunes (sala, comedor, cocina, etc.) y la zona privada (habitaciones) ya que cada una requiere una cantidad de iluminación diferente",
    "explanation": "Es aquel recurso natural que incide positivamente en los espacios. Se utiliza con el fin de optimizar la utilización de energías no renovables como la iluminación artificial.",
    "question": "¿Cómo percibes la iluminación natural al interior de las habitaciones?",
    "options": {
      "Buena": 0.02,
      "Regular": 0.01,
      "Deficiente": 0
    },
    "imageId": 35,
    "parameterType": "Regular"
  },
  {
    "id": 36,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Ventilación",
    "measurement": "Se busca que el usuario concentre su atención en el tipo de ventilación que poseen los baños de la unidad o la ausencia de ésta, además del resto de la vivienda.",
    "explanation": "Circulación de partículas de aire al interior de la vivienda (entradas y salidas). Representa una de las principales determinantes del diseño arquitectónico dado que una vivienda mal ventilada puede afectar la salud de quienes la habitan",
    "question": "¿Cómo percibes la ventilación al interior de la vivienda?",
    "options": {
      "Buena": 0.03,
      "Regular": 0.015,
      "Deficiente": 0
    },
    "imageId": 36,
    "parameterType": "Regular"
  },
  {
    "id": 37,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Ventilación",
    "measurement": "Se busca que el usuario concentre su atención en el tipo de ventilación que poseen los baños de la unidad o la ausencia de ésta, además del resto de la vivienda.",
    "explanation": "Circulación de partículas de aire al interior de la vivienda (entradas y salidas). Representa una de las principales determinantes del diseño arquitectónico dado que una vivienda mal ventilada puede afectar la salud de quienes la habitan",
    "question": "¿Cuentan con ventilación natural o artificial los baños de la vivienda?",
    "options": {
      "Natural": 0.03,
      "Artificial": 0.015,
      "No tiene ventilación": 0
    },
    "imageId": 37,
    "parameterType": "Regular"
  },
  {
    "id": 38,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Instalaciones",
    "measurement": "Este parámetro se enfocó en las instalaciones hidráulicas, con un mínimo de una llave de paso independiente en un punto húmedo (baños y zonas húmedas), e instalaciones eléctricas, con un mínimo de 5 circuitos protegidos por breakers.",
    "explanation": "Sistemas de tipo eléctrico, sanitario, entre otros, que nos posibilitan el acceso a los servicios básicos (electricidad, agua, gas, entre otros).",
    "question": "¿Cuenta con llaves de paso independientes en los puntos húmedos?",
    "options": {
      "No": 0,
      "Si": 0.03
    },
    "imageId": 38,
    "parameterType": "Regular"
  },
  {
    "id": 39,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Instalaciones",
    "measurement": "Este parámetro se enfocó en las instalaciones hidráulicas, con un mínimo de una llave de paso independiente en un punto húmedo (baños y zonas húmedas), e instalaciones eléctricas, con un mínimo de 5 circuitos protegidos por breakers.",
    "explanation": "Sistemas de tipo eléctrico, sanitario, entre otros, que nos posibilitan el acceso a los servicios básicos (electricidad, agua, gas, entre otros).",
    "question": "¿Cuenta con mínimo 5 o 6 breakers/tacos el sistema eléctrico de la vivienda?",
    "options": {
      "No": 0,
      "Si": 0.03
    },
    "imageId": 39,
    "parameterType": "Regular"
  },
  {
    "id": 40,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Almacenamiento",
    "measurement": "Las preguntas orientadoras de este parámetro van enfocadas hacia la cantidad de espacio destinado para guardar objetos, el usuario debe prestar especial atención hacia cuánto espacio de almacenamiento, como closet, alacenas, cuartos de depósito, etc. posee la vivienda.",
    "explanation": "El almacenamiento en una vivienda se determina como todos aquellos espacios u objetos en los cuales podemos guardar pertenencias e insumos.",
    "question": "¿La vivienda cuenta con espacios de almacenamiento suficientes? (Closet, cuarto de deposito, muebles de cocina, alacenas)",
    "options": {
      "No": 0,
      "Si": 0.02
    },
    "imageId": 40,
    "parameterType": "Regular"
  },
  {
    "id": 41,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Asoleamiento",
    "measurement": "En este caso la incidencia directa del sol por un periodo de tiempo largo es causante de altas temperaturas al interior de la vivienda, es por esto que la ausencia de los rayos solares en todos los espacios de la unidad en horas críticas (9:00 - 16:00) aportan calidad a la residencia",
    "explanation": "En arquitectura el soleamiento se considera como la incidencia del sol en el objeto arquitectónico.",
    "question": "¿En qué jornada observas la incidencia directa de la luz solar en las alcobas (espacios privados)?",
    "options": {
      "AM": 0.04,
      "PM": 0.01,
      "AM Y PM": 0,
      "Ninguno": 0.02
    },
    "imageId": 41,
    "parameterType": "Regular"
  },
  {
    "id": 42,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Asoleamiento",
    "measurement": "En este caso la incidencia directa del sol por un periodo de tiempo largo es causante de altas temperaturas al interior de la vivienda, es por esto que la ausencia de los rayos solares en todos los espacios de la unidad en horas críticas (9:00 - 16:00) aportan calidad a la residencia",
    "explanation": "En arquitectura el soleamiento se considera como la incidencia del sol en el objeto arquitectónico.",
    "question": "¿En qué jornada observas la incidencia directa de la luz solar en alguno de los siguientes espacios: sala, comedor, cocina y baños (espacios sociales)?",
    "options": {
      "AM": 0.04,
      "PM": 0.01,
      "AM Y PM": 0,
      "Ninguno": 0.02
    },
    "imageId": 42,
    "parameterType": "Regular"
  },
  {
    "id": 43,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Acabados funcionales",
    "measurement": "",
    "explanation": "Es aquel elemento/material constructivo que permite mantener el estado de la vivienda útil por lapsos de tiempo más prolongados.",
    "question": "¿La vivienda cuenta con los acabados esenciales? (Ej: Enchape de baños, enchape sobre encimera de cocina, enchape de pisos)",
    "options": {
      "No": 0,
      "Si": 0.06
    },
    "imageId": 43,
    "parameterType": "Regular"
  },
  {
    "id": 44,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Domótica",
    "measurement": "",
    "explanation": "Permite automatizar las tareas relacionadas con la seguridad, el bienestar y el confort mediante un sistema inteligente instalado en una vivienda o edificio.",
    "question": "¿La  vivienda cuenta con un sistema de automatización como iluminacion inteligente, climatizacion automatizada, seguridad y acceso, persianas y cortinas automatizadas, electrodomesticos y entretenimiento?",
    "options": {
      "No": 0,
      "Si": 1
    },
    "imageId": 44,
    "parameterType": "Plus",
    "parameterPlusTitle": "Domótica"
  },
  {
    "id": 45,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Dimensiones funcionales",
    "measurement": "Dentro de este parámetro se quiere evaluar que los mobiliarios fijos de la vivienda estén ubicados donde puedan permitirle al usuario hacer sus actividades correctamente y sin barreras, ejemplo: tener que cerrar una puerta para poder acceder del todo a un espacio.",
    "explanation": "Va ligado al término ergonomía, siendo la disciplina que se encarga de establecer espacios útiles de acuerdo a las características físicas y psicológicas en este caso del ser humano en la vivienda.",
    "question": "¿Los mobiliarios fijos interfieren con la circulación o el desarrollo de otra actividad? (Aparatos sanitarios, puertas y ventanas, entre otros)",
    "options": {
      "Si": 0.04,
      "Algunos": 0.02,
      "No": 0
    },
    "imageId": 45,
    "parameterType": "Regular"
  },
  {
    "id": 46,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Flexibilidad",
    "measurement": "La pregunta orientadora en este parámetro hace referencia a la capacidad que le otorga el espacio al usuario de cambiar la posición de sus mobiliarios y diseño interior a uno completamente diferente, sin que se vea afectada la circulación el lugar. Ejemplo: tener un espacio donde la cama solo pueda estar ubicada en el centro de la habitación",
    "explanation": "El término flexibilidad está ligado a qué tan maleables son los espacios de la vivienda, de manera que nos permitan jugar y/o variar la posición de los mobiliarios. ",
    "question": "¿Los diferentes espacios de la vivienda le permiten cambiar la posición de los mobiliarios?",
    "options": {
      "Si": 0.04,
      "Algunos": 0.02,
      "No": 0
    },
    "imageId": 46,
    "parameterType": "Regular"
  },
  {
    "id": 47,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Espacios",
    "measurement": "Las preguntas orientadoras en este parámetro buscan incluir las preferencias del usuario en el puntaje de calidad de su vivienda, como las habitaciones que desea, los espacios adicionales que posee la casa, etcEn este caso nos enfocados en los acabados esenciales con los que debe contar una vivienda, pisos, baños y cocina",
    "explanation": "Diferentes medios físicos que permiten el desarrollo de actividades cotidianas para el ser humano como dormir, asearse, recrearse, entre otros.",
    "question": "¿La vivienda cuenta con los espacios mínimos para ser habitable? (1 baño, cocina, zona de labores, sala/comedor, 1 habitación)",
    "options": {
      "No": 0,
      "Si": 0.0267
    },
    "imageId": 47,
    "parameterType": "Regular"
  },
  {
    "id": 48,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Espacios",
    "measurement": "Las preguntas orientadoras en este parámetro buscan incluir las preferencias del usuario en el puntaje de calidad de su vivienda, como las habitaciones que desea, los espacios adicionales que posee la casa, etcEn este caso nos enfocados en los acabados esenciales con los que debe contar una vivienda, pisos, baños y cocina",
    "explanation": "Diferentes medios físicos que permiten el desarrollo de actividades cotidianas para el ser humano como dormir, asearse, recrearse, entre otros.",
    "question": "¿Cuenta la vivienda con espacios adicionales como sala de TV (Independiente a sala/comedor), patio de ropas, estudio, balcón?",
    "options": {
      "Todos": 0.0267,
      "Algunos": 0.0135,
      "Ninguno": 0
    },
    "imageId": 48,
    "parameterType": "Regular"
  },
  {
    "id": 49,
    "indicator": "Unidad Residencial",
    "subgroup": "Utilidad",
    "parameter": "Espacios",
    "measurement": "Las preguntas orientadoras en este parámetro buscan incluir las preferencias del usuario en el puntaje de calidad de su vivienda, como las habitaciones que desea, los espacios adicionales que posee la casa, etcEn este caso nos enfocados en los acabados esenciales con los que debe contar una vivienda, pisos, baños y cocina",
    "explanation": "Diferentes medios físicos que permiten el desarrollo de actividades cotidianas para el ser humano como dormir, asearse, recrearse, entre otros.",
    "question": "¿Cuenta con parqueadero propio?",
    "options": {
      "No": 0,
      "Si": 0.0267
    },
    "imageId": 49,
    "parameterType": "Regular"
  },
  {
    "id": 50,
    "indicator": "Unidad Residencial",
    "subgroup": "Deleite",
    "parameter": "Calidad de acabados",
    "measurement": "En este caso nos enfocados en los acabados esenciales con los que debe contar una vivienda, pisos, baños y cocina",
    "explanation": "Es aquel elemento/material constructivo que permite mantener el estado de la vivienda útil por lapsos de tiempo más prolongados.",
    "question": "¿Qué tipo de acabado tienen los muros de la vivienda?",
    "options": {
      "Estuco y pintura": 0.04,
      "Ladrillo o bloque": 0,
      "Concreto/mortero a la vista": 0.02
    },
    "imageId": 50,
    "parameterType": "Regular"
  },
  {
    "id": 51,
    "indicator": "Unidad Residencial",
    "subgroup": "Deleite",
    "parameter": "Calidad de acabados",
    "measurement": "En este caso nos enfocados en los acabados esenciales con los que debe contar una vivienda, pisos, baños y cocina",
    "explanation": "Es aquel elemento/material constructivo que permite mantener el estado de la vivienda útil por lapsos de tiempo más prolongados.",
    "question": "¿Qué tipo de acabado tienen los pisos de la vivienda?",
    "options": {
      "Baldosas / Cerámica": 0.04,
      "Sin acabado (piso en cemento o plantilla)": 0
    },
    "imageId": 51,
    "parameterType": "Regular"
  },
  {
    "id": 52,
    "indicator": "Unidad Residencial",
    "subgroup": "Solidez",
    "parameter": "Mantenimiento/Patologias",
    "measurement": "",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿Los muros o columnas tienen grietas o fisuras?",
    "options": {
      "No tienen fisuras ni grietas": 20,
      "Tienen pequeñas fisuras delgadas menores a 1mm": 15,
      "Tienen fisuras de 1mm a 3mm de ancho, pero no profunda": 10,
      "Grietas visibles mayores a 3mm o algunas separaciones en muros": 5,
      "Grandes grietas que atraviesan muros o columnas, riesgo estructural": 0
    },
    "imageId": 52,
    "parameterType": "Depreciation"
  },
  {
    "id": 53,
    "indicator": "Unidad Residencial",
    "subgroup": "Solidez",
    "parameter": "Mantenimiento/Patologias",
    "measurement": "",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "La cubierta tiene filtraciones o daños?",
    "options": {
      "No hay filtraciones, el techo está en perfecto estado": 20,
      "Hay manchas pequeñas en el techo, pero no gotea": 15,
      "Hay goteras ocasionales cuando llueve mucho": 10,
      "Gotea constantemente en varias zonas": 5,
      "El techo está muy deteriorado o tiene riesgo de colapso": 0
    },
    "imageId": 53,
    "parameterType": "Depreciation"
  },
  {
    "id": 54,
    "indicator": "Unidad Residencial",
    "subgroup": "Solidez",
    "parameter": "Mantenimiento/Patologias",
    "measurement": "",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿Cuál es el estado de los pisos, paredes y puertas?",
    "options": {
      "Todos los acabados están en excelente estado, sin daños": 20,
      "Algunas paredes o pisos tienen desgaste leve, pero son funcionales": 15,
      "Algunas baldosas o puertas tienen daños, pero pueden usarse": 10,
      "Muchos pisos o puertas están dañados, hay desgaste evidente": 5,
      "Los acabados están en mal estado o en partes inexistentes": 0,
    },
    "imageId": 54,
    "parameterType": "Depreciation"
  },
  {
    "id": 55,
    "indicator": "Unidad Residencial",
    "subgroup": "Solidez",
    "parameter": "Mantenimiento/Patologias",
    "measurement": "",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿Cómo funcionan los enchufes, luces y tuberías de agua?",
    "options": {
      "Todo funciona perfectamente, sin fallas": 20,
      "Pequeños problemas ocasionales que no afectan el uso": 15,
      "Algunas conexiones fallan a veces o hay fugas de agua menores": 10,
      "Se dañan frecuentemente, hay fugas o cortocircuitos": 5,
      "Instalaciones en mal estado, fallas graves o tuberías rotas": 0,
    },
    "imageId": 55,
    "parameterType": "Depreciation"
  },
  {
    "id": 56,
    "indicator": "Unidad Residencial",
    "subgroup": "Solidez",
    "parameter": "Mantenimiento/Patologias",
    "measurement": "",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿La vivienda necesita mantenimiento urgente?",
    "options": {
      "Todo funciona perfectamente, sin fallas": 20,
      "Pequeños problemas ocasionales que no afectan el uso": 15,
      "Algunas conexiones fallan a veces o hay fugas de agua menores": 10,
      "Se dañan frecuentemente, hay fugas o cortocircuitos": 5,
      "Instalaciones en mal estado, fallas graves o tuberías rotas": 0,
    },
    "imageId": 56,
    "parameterType": "Depreciation"
  },
  {
    "id": 57,
    "indicator": "Unidad Residencial",
    "subgroup": "Sostenibilidad",
    "parameter": "Sostenibilidad Ambiental",
    "measurement": "En este caso el recurso en el que se enfocó este parámetro es el agua, es decir, como reutilizarla y almacenarla de manera adecuada y responsable.",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿Cuenta con algún sistema de recolección o reutilizacion de aguas lluvias? ",
    "options": {
      "No": 0,
      "Si": 0.008
    },
    "imageId": 57,
    "parameterType": "Regular"
  },
  {
    "id": 58,
    "indicator": "Unidad Residencial",
    "subgroup": "Sostenibilidad",
    "parameter": "Sostenibilidad Ambiental",
    "measurement": "En este caso el recurso en el que se enfocó este parámetro es el agua, es decir, como reutilizarla y almacenarla de manera adecuada y responsable.",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿La vivienda cuenta iluminación eficiente y sostenible, por ejemplo lámparas LED?",
    "options": {
      "No": 0,
      "Si": 0.008
    },
    "imageId": 58,
    "parameterType": "Regular"
  },
  {
    "id": 59,
    "indicator": "Unidad Residencial",
    "subgroup": "Sostenibilidad",
    "parameter": "Sostenibilidad Ambiental",
    "measurement": "En este caso el recurso en el que se enfocó este parámetro es el agua, es decir, como reutilizarla y almacenarla de manera adecuada y responsable.",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿Existen dispositivos de ahorro de agua en grifos, duchas e inodoros ?",
    "options": {
      "No": 0,
      "Si": 0.008
    },
    "imageId": 59,
    "parameterType": "Regular"
  },
  {
    "id": 60,
    "indicator": "Unidad Residencial",
    "subgroup": "Sostenibilidad",
    "parameter": "Sostenibilidad Ambiental",
    "measurement": "En este caso el recurso en el que se enfocó este parámetro es el agua, es decir, como reutilizarla y almacenarla de manera adecuada y responsable.",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿Posee un techo verde o jardin vertical, que contribuya a la reducción de el calor y la mejora de la calidad del aire?",
    "options": {
      "No": 0,
      "Si": 0.008
    },
    "imageId": 60,
    "parameterType": "Regular"
  },
  {
    "id": 61,
    "indicator": "Unidad Residencial",
    "subgroup": "Sostenibilidad",
    "parameter": "Sostenibilidad Ambiental",
    "measurement": "En este caso el recurso en el que se enfocó este parámetro es el agua, es decir, como reutilizarla y almacenarla de manera adecuada y responsable.",
    "explanation": "Se enfoca en la optimización de recursos a través de sistemas que nos permitan la reutilización de estos en otras áreas o actividades",
    "question": "¿La vivienda cuenta con sistemas de recolección de energía (ya sea solar, eólica, entre otras)?",
    "options": {
      "No": 0,
      "Si": 0.008
    },
    "imageId": 61,
    "parameterType": "Regular"
  }
];